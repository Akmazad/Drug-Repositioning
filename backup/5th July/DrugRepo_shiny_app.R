...
server <- function(input, output, session) {
  ...
  output$downloadDSN <- downloadHandler(
    ...
    content = function(file) {
      file.copy("./data/net-information - Copy - Copy.csv", file)
    }
  )
  ...
  DSN <- read.csv("./data/net-information - Copy - Copy.csv",header = TRUE,sep = ",", stringsAsFactors = FALSE)
  ...
  observeEvent(input$select_button, {
    ...
    mytabSN$Value <<- as.data.frame(unique(DSN[which(DSN$ID1 == input$select_button),-c(1,2)]))
    groups = drugMetaData[which(drugMetaData[,1] %in% mytabSN$Value[,1]),c(1,3)] 
    groups[,2] = gsub("\\,",", ", groups[,2])
    colnames(groups) <- c("id","Groups")
    mytabSN$Value[,5] = ifelse(is.na(mytabSN$Value[,5]),"NA",mytabSN$Value[,5])
    
    colnames(mytabSN$Value) = c("id", "Target",  "Structure Similarity", "Target Similarity", 
                                "Pathway Similarity", "p-Value")
    mytabSN$Value = merge(mytabSN$Value,groups,by = "id") 
    mytabSN$Value = mytabSN$Value[,-1]
